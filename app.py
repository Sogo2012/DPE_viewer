...
import io # Not explicitly used in the final version but often useful with image/file handling
import datetime
import base64
import os
...
LOGO_DIRECTORY = "assets" # Make sure this 'assets' folder is in the same directory as app.py

SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
LOGO_PATH = os.path.join(SCRIPT_DIR, LOGO_DIRECTORY, LOGO_FILENAME)

# Control para imprimir debug del logo solo una vez
if 'logo_debug_printed' not in st.session_state:
    print(f"--- DEBUG INFO (UNA SOLA VEZ) ---")
    print(f"Ruta del script actual (__file__): {os.path.abspath(__file__)}")
    print(f"Directorio del script (SCRIPT_DIR): {SCRIPT_DIR}")
    print(f"Ruta construida para el Logo (LOGO_PATH): {LOGO_PATH}")
    print(f"¿Existe el archivo en LOGO_PATH? {os.path.exists(LOGO_PATH)}")
    print(f"Directorio de trabajo actual (cwd): {os.getcwd()}")
    print(f"--- FIN DEBUG INFO ---")
    st.session_state.logo_debug_printed = True
...
def get_image_as_base64(path):
    try:
        ...
        # print(f"ERROR (get_image_as_base64): Archivo no encontrado en {os.path.abspath(path)}") # Comentado para menos ruido
        return None
    except Exception as e:
        # print(f"ERROR SILENCIADO (get_image_as_base64): {e}") # Log discreto en consola
        return None
...
try:
    ...
    page_icon_img = "📊"
    # print(f"ADVERTENCIA (Favicon): Error al abrir el logo '{LOGO_PATH}': {e}. Usando emoji.")
...
<style>
    ...
    h1 {{ color: {COLOR_TEXTO_TITULO_PRINCIPAL}; padding-bottom: 0.5rem; border-bottom: 3px solid {COLOR_AZUL_ECO}; }}
    h2 {{ color: {COLOR_TEXTO_SUBTITULO_SECCION}; border-bottom: 2px solid {COLOR_VERDE_ECO}; padding-bottom: 0.3rem; margin-top: 2rem; }}
    h3 {{ color: {COLOR_TEXTO_SUB_SUBTITULO}; margin-top: 1.5rem; }}
    p, div, span, li, .stMarkdown {{ color: {COLOR_TEXTO_CUERPO}; line-height: 1.6; }}
    hr {{ margin-top: 0.5rem; margin-bottom: 1rem; border: 0; border-top: 1px solid #D5D8DC; }}

    /* Estilos para el Radio Button de la Sidebar */
    div[data-testid="stRadio"] label p {{ /* Asegurar que el texto dentro del label tenga el color correcto */
        color: {COLOR_TEXTO_CUERPO};
    }}
    div[data-testid="stRadio"] label {{
        display: block; padding: 8px 10px; margin-bottom: 5px; border-radius: 4px;
        transition: background-color 0.2s ease; cursor: pointer;
    }}
    div[data-testid="stRadio"] label:hover {{ background-color: #e9ecef; }}
    div[data-testid="stRadio"] input[type="radio"]:checked + div {{
        background-color: {COLOR_VERDE_ECO}; color: {COLOR_TEXTO_BLANCO}; font-weight: bold;
    }}
...
if 'seccion_actual' not in st.session_state:
    st.session_state.seccion_actual = "Portada"
...
with st.sidebar:
    if logo_base64:
        ...
        if 'logo_warning_sidebar_printed' not in st.session_state: # Evitar spam en consola
            print(f"ADVERTENCIA (Sidebar): Logo no encontrado en '{LOGO_PATH}'. Mostrando texto.")
            st.session_state.logo_warning_sidebar_printed = True
    ...
    if uploaded_file is not None:
        with st.spinner("Procesando archivo JSON..."):
            try:
                ...
                st.session_state.seccion_actual = "Portada" # Reset to Portada on new upload
    ...
    if st.session_state.json_data:
        ...
        if st.button("🧹 Limpiar Datos y Reiniciar"):
            show_json_toggle_state_saved = st.session_state.get('show_json_data', False)
            # Guardar el estado de seccion_actual podría ser útil si no se quiere forzar a Portada
            # seccion_guardada = st.session_state.get('seccion_actual', "Portada")

            keys_to_delete = list(st.session_state.keys())
            for key in keys_to_delete:
                if key not in ['logo_debug_printed', 'logo_warning_sidebar_printed']: # No borrar estas flags
                    del st.session_state[key]
            
            # Restaurar/Reinicializar estados necesarios
            st.session_state.show_json_data = show_json_toggle_state_saved
            st.session_state.seccion_actual = "Portada" # Forzar a Portada al limpiar
            st.session_state.json_data = None # Asegurarse que json_data sea None para pantalla de bienvenida
            st.session_state.nombre_cliente = "Cliente"
            st.session_state.error_carga = None
            st.rerun()
        ...
        st.markdown("---")
        st.sidebar.header("Navegación del Informe")
        
        mapa_secciones_sidebar = {
            "I. Portada": "Portada", "II. Glosario": "Glosario", "III. Resumen": "Resumen Ejecutivo",
            "IV. Intro": "Introducción", "V. A. Externo": "Análisis Externo", "VI. D. Interno": "Diagnóstico Interno",
            "VII. FODA": "Síntesis FODA", "VIII. Form. Estrat.": "Formulación Estratégica",
            "IX. Hoja de Ruta": "Hoja de Ruta", "X. Implement.": "Implementación", "XI. Conclusiones": "Conclusiones"
        }
        lista_secciones_sidebar_titulos_cortos = list(mapa_secciones_sidebar.keys())
        
        seccion_actual_larga_sidebar = st.session_state.get('seccion_actual', "Portada")
        index_actual_sidebar = 0
        try:
            # Encontrar el índice del título corto correspondiente a la sección actual larga
            # Esto es para que el radio button se inicialice en la sección correcta
            lista_secciones_largas_mapeadas = [mapa_secciones_sidebar[k] for k in lista_secciones_sidebar_titulos_cortos]
            if seccion_actual_larga_sidebar in lista_secciones_largas_mapeadas:
                index_actual_sidebar = lista_secciones_largas_mapeadas.index(seccion_actual_larga_sidebar)
        except ValueError:
            index_actual_sidebar = 0 # Default to first item if not found

        seccion_sidebar_seleccionada_corta = st.sidebar.radio(
            "Seleccione una sección:",
            options=lista_secciones_sidebar_titulos_cortos,
            index=index_actual_sidebar,
            key="navegacion_informe_radio_sidebar" 
        )
        
        # Actualizar la sección solo si ha cambiado para evitar reruns innecesarios
        if seccion_sidebar_seleccionada_corta and \
           mapa_secciones_sidebar[seccion_sidebar_seleccionada_corta] != st.session_state.seccion_actual:
            st.session_state.seccion_actual = mapa_secciones_sidebar[seccion_sidebar_seleccionada_corta]
            st.rerun()
...
def render_portada(data):
    ...
    try:
        img_portada = Image.open(LOGO_PATH) # Reusa el logo de la empresa para la portada, o define una imagen específica
        ...
        st.markdown(f"<p style='font-size: 0.8em; color: {COLOR_TEXTO_SUTIL};'>(Error al cargar logo de portada: {e})</p>", unsafe_allow_html=True)
...
def render_glosario(data):
    st.header(data.get('titulo_seccion_texto', 'X. Glosario de Términos')) # El PDF lo tiene como X.
    st.markdown("---")
    ...
    for item in data.get("lista_terminos_data", []):
        ...
        with st.container():
            ...
            st.markdown("---", help="Separador de término") # El help es opcional
...
def render_resumen_ejecutivo(data_re):
    ...
    if radar_data_list and isinstance(radar_data_list, list) and len(radar_data_list) >= 3:
        ...
        for v_str in values_str:
            ...
            except (ValueError, TypeError): values.append(0.0) # Default a 0 si no es numérico
        ...
        if labels and values and len(labels) == len(values):
            ...
            fig_radar.add_trace(go.Scatterpolar(
                r=values + ([values[0]] if values else []), # Cerrar el radar
                theta=labels + ([labels[0]] if labels else []), # Cerrar el radar
            ...
            st.caption(sec_madurez_global.get("grafico_radar_caption_texto", 
                                              sec_madurez_global.get("grafico_radar_titulo_sugerido", ""))) # Reusar título como caption si no hay caption
        else:
    ...
    # Iterar sobre las subsecciones restantes del resumen ejecutivo
    ...
    for sub_key in sub_secciones_resumen:
        ...
        if sub_data: # Solo renderizar si la subsección existe
            st.subheader(sub_data.get('subtitulo_texto', sub_key.replace("_", " ").title())) # Título genérico si falta
            ...
            # Listas de hallazgos, fortalezas, etc.
            ...
            st.markdown("---") # Separador después de cada subsección del resumen
...
def render_introduccion_contexto(data):
    ...
    for key, val in seccion_presentacion.items():
        if key not in ['subtitulo_texto', 'titulo_seccion_texto'] and isinstance(val, str): # Evitar imprimir el propio subtítulo
    ...
    st.markdown(f"**{seccion_alcance.get('proceso_recoleccion_titulo_texto','Proceso de Recolección y Marco de Evaluación:')}**") # Título más descriptivo
...
def render_analisis_externo(data):
    ...
    if "grafico_bccr_data" in sec_macro and sec_macro["grafico_bccr_data"]:
        ...
        if not df_bccr.empty and 'Fecha' in df_bccr.columns:
            try:
                ...
                tbp_col = 'Tasa_Basica_Pasiva' # Suponiendo estos nombres de columna
                ...
                if tbp_data_exists or tc_data_exists:
                    fig_bccr.update_layout(
                        ...
                        yaxis=dict(title=dict(text='Tasa Básica Pasiva (%)', font=dict(color=COLOR_AZUL_ECO)), 
                                   ...
                                   visible=tbp_data_exists), # Solo visible si hay datos TBP
                        yaxis2=dict(title=dict(text='Tipo de Cambio Venta (CRC)', font=dict(color=COLOR_VERDE_ECO)), 
                                    ...
                                    showgrid=tc_data_exists, gridcolor='rgba(0,0,0,0.05)', # Grid solo si hay datos TC
                                    visible=tc_data_exists), # Solo visible si hay datos TC
                        legend_title_text='Indicadores', legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1),
                        paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', font_color=COLOR_TEXTO_CUERPO
                    )
                ...
                # st.json(sec_macro["grafico_bccr_data"]) # Descomentar para depurar datos si es necesario
        else:
            st.info("Datos para gráfico BCCR no disponibles o en formato incorrecto (falta columna 'Fecha' o datos vacíos).")
    else:
        st.info("No se encontraron datos para el gráfico BCCR en 'macroentorno'.")
    st.markdown("---")

    # B. Sector/Industria (CFIA M2)
    ...
    if tend_data and tend_data.get("historico") and tend_data.get("actual_proyeccion"):
        try:
            ...
            df_real = pd.DataFrame(tend_data.get("actual_real", [])) # Puede estar vacío
            ...
            if not df_hist.empty and 'Mes' in df_hist.columns:
                for col in df_hist.columns:
                    if col != 'Mes': # Asume que otras columnas son años/categorías
            ...
            if not df_real.empty and 'Mes' in df_real.columns and 'Valor_Actual' in df_real.columns:
                ...
                df_real_plot = df_real.dropna(subset=['Valor_Actual']) # Plot only valid numbers
            ...
            # Proyección (conectada o completa)
            if not df_proy.empty and 'Mes' in df_proy.columns and 'Valor_Proyeccion' in df_proy.columns:
                ...
                if last_real_month_name and last_real_value is not None: # Si hay datos reales para conectar
                    if last_real_month_name in df_proy_plot_final['Mes'].values:
                        ...
                        if not idx_proy_start.empty:
                            ...
                            # Crear df temporal para la proyección que empieza desde el punto de conexión
            ...
            # st.json(tend_data) # Descomentar para depuración
    else:
        st.info("Datos para gráfico de tendencia M2 (CFIA) no disponibles o incompletos.")
    # Gráficos adicionales de CFIA (Variación Provincial, Acumulados) se omiten si no hay datos o es complejo con datos de ejemplo
    # Si los datos para estos gráficos están en `sec_cfia` bajo claves como `grafico_variacion_provincial_data`
    # y `grafico_acumulados_provincia_data`, se pueden añadir de forma similar.
    ...
    for i, comp in enumerate(sec_comp.get('lista_competidores_data', [])):
        # Tratar de extraer un nombre más limpio para el expander
        ...
        with st.expander(f"{comp.get('id_competidor_display_texto', f'Competidor {i+1}')}: {nombre_competidor}", expanded=i==0): # Expandir el primero por defecto
...
def render_diagnostico_interno(data):
    ...
    for area_data in sec_eval_areas.get('lista_areas_evaluacion_data', []):
        ...
        with st.expander(area_titulo_display, expanded=True): # Expandir todas por defecto, o cambiar a False
            ...
            st.write(area_data.get('oportunidades_mejora_resumen_texto', "")) # Este suele ser un párrafo
    st.markdown("---")
...
def render_sintesis_foda(data):
    ...
    if "(Placeholder:" in placeholder_tows or not sec_tows.get('parrafo_texto_analisis_FO') : # Chequear si es placeholder o si falta contenido específico
         st.info(placeholder_tows) # Muestra el texto de sesión de trabajo si es placeholder
    else: # Si hay contenido específico, mostrarlo
...
def render_formulacion_estrategica(data):
    ...
    for i, obj_data in enumerate(sec_obj.get('lista_objetivos_data', [])):
        ...
        if i < len(sec_obj.get('lista_objetivos_data', [])) - 1: st.markdown("---") # Separador entre objetivos
    st.markdown("---")
...
def render_hoja_ruta(data):
    ...
    for obj_hr in sec_detalle.get('lista_objetivos_con_detalle_data', []):
        ...
        for inic_hr in obj_hr.get('iniciativas_estrategicas_data', []):
            with st.container(): # Usar container para agrupar visualmente cada iniciativa
                ...
                st.markdown("---") # Separador entre iniciativas
    st.markdown("---")

    sec_cron = data.get("cronograma_general_hoja_ruta", {})
    st.subheader(sec_cron.get('subtitulo_texto', "C. Cronograma General de la Hoja de Ruta (Visualización Preliminar)")) # PDF lo tiene como D.
...
def render_implementacion(data):
    ...
    if not lista_factores or "(Placeholder:" in sec_factores.get('parrafo_intro_texto', lista_factores[0] if lista_factores else ""):
        st.info(sec_factores.get('parrafo_intro_texto', texto_sesion_trabajo)) # Muestra el placeholder o texto de sesión
    else:
    ...
    # Chequear si el primer riesgo es un placeholder o si la lista está vacía
...
def render_conclusiones(data):
    ...
    if is_placeholder_rec90:
        st.info(sec_rec90.get('parrafo_intro_texto', texto_sesion_trabajo)) # Usa el parrafo_intro_texto si es un placeholder
    else:
...
if st.session_state.json_data is None:
    ...
    # Lógica de renderizado basada en st.session_state.seccion_actual (de la sidebar)
    seccion_a_mostrar = st.session_state.get('seccion_actual', "Portada")

    # Mapeo de sección actual a función de renderizado y clave de datos JSON
    mapa_renderizado = {
        "Portada": (render_portada, "portada"),
        "Glosario": (render_glosario, "glosario"),
        "Resumen Ejecutivo": (render_resumen_ejecutivo, "resumen_ejecutivo"),
        "Introducción": (render_introduccion_contexto, "introduccion_contexto"),
        "Análisis Externo": (render_analisis_externo, "analisis_entorno_externo"),
        "Diagnóstico Interno": (render_diagnostico_interno, "diagnostico_interno"),
        "Síntesis FODA": (render_sintesis_foda, "sintesis_estrategica_foda"),
        "Formulación Estratégica": (render_formulacion_estrategica, "formulacion_estrategica"),
        "Hoja de Ruta": (render_hoja_ruta, "hoja_ruta_estrategica"),
        "Implementación": (render_implementacion, "consideraciones_implementacion"),
        "Conclusiones": (render_conclusiones, "conclusiones_finales"),
    }

    if seccion_a_mostrar in mapa_renderizado:
        render_function, data_key = mapa_renderizado[seccion_a_mostrar]
        data_for_section = json_data_main.get(data_key, {})
        if data_for_section:
            render_function(data_for_section)
        else:
            st.warning(f"Datos para la sección '{seccion_a_mostrar}' (clave: '{data_key}') no encontrados en el archivo JSON.")
            # Intentar renderizar con un diccionario vacío para que la función muestre sus propios mensajes de "no datos"
            render_function({}) 
    else:
        st.error(f"Sección '{seccion_a_mostrar}' no reconocida. Mostrando Portada por defecto.")
        render_portada(json_data_main.get("portada", {}))
