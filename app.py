...
# import io # No se usa directamente en esta versión
import datetime
import base64
import os
...
LOGO_FILENAME = "Logo_ECO.png"  # Asegúrate que coincida EXACTAMENTE con el nombre en GitHub
LOGO_DIRECTORY = "assets"     # Asegúrate que coincida EXACTAMENTE con el nombre en GitHub

# Esta es la forma más robusta de obtener la ruta al script
SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
LOGO_PATH = os.path.join(SCRIPT_DIR, LOGO_DIRECTORY, LOGO_FILENAME)

# --- Variables para depuración del logo ---
# Estas se evaluarán una vez al inicio del script.
logo_path_calculated = LOGO_PATH
logo_exists_at_path = os.path.exists(logo_path_calculated)
current_working_directory = os.getcwd()
# --- Fin Variables para depuración ---

def get_image_as_base64(path):
    if not os.path.exists(path): # Re-chequeo por si acaso
        print(f"ERROR (get_image_as_base64): Archivo no encontrado en ruta absoluta: {os.path.abspath(path)}")
        return None
    try:
        with open(path, "rb") as img_file:
            return base64.b64encode(img_file.read()).decode()
    except Exception as e:
        print(f"ERROR (get_image_as_base64) procesando {path}: {e}")
        return None

logo_base64 = get_image_as_base64(logo_path_calculated) # Usar la ruta calculada

page_icon_img_to_set = "📊" # Fallback
try:
    if logo_exists_at_path: # Usar la variable pre-calculada
        page_icon_img_to_set = Image.open(logo_path_calculated)
    # else: el fallback "📊" se mantiene
except Exception as e:
    print(f"ADVERTENCIA (Favicon): Error al abrir logo '{logo_path_calculated}': {e}. Usando emoji.")
    pass # El fallback "📊" se mantiene

st.set_page_config(
    page_title=APP_TITLE,
    page_icon=page_icon_img_to_set, # Puede ser un objeto Image o un emoji
    layout="wide",
...
with st.sidebar:
    ...
    st.text(f"SCRIPT_DIR: ...{SCRIPT_DIR[-30:]}") # Mostrar últimos 30 chars
    st.text(f"LOGO_PATH: ...{logo_path_calculated[-40:]}") # Mostrar últimos 40 chars
    st.text(f"os.getcwd(): ...{current_working_directory[-30:]}")
    if logo_exists_at_path:
        st.success(f"Logo DEBERÍA existir en LOGO_PATH.")
    else:
        st.error(f"Logo NO encontrado en LOGO_PATH.")
    
    if logo_base64:
        st.success("logo_base64 cargado.")
    else:
        st.warning("logo_base64 es None (no se pudo cargar).")
    st.markdown("---") # Fin sección Debug
    ...
    if logo_base64:
        ...
        if 'logo_warning_sidebar_printed' not in st.session_state:
            print(f"ADVERTENCIA (Sidebar): logo_base64 es None. Mostrando texto 'ECO Consultores'. Ruta intentada: '{logo_path_calculated}'")
    ...
    if st.session_state.json_data:
        ...
        if st.button("🧹 Limpiar Datos y Reiniciar"):
            # ... (lógica de limpiar, igual que antes)
            show_json_toggle_state_saved = st.session_state.get('show_json_data', False)
            keys_to_delete = list(st.session_state.keys())
            for key in keys_to_delete:
                if key not in ['logo_warning_sidebar_printed']: # Mantener flags de "ya impreso"
...
# (Estas funciones son las mismas que en la respuesta anterior, asumiendo que
#  LOGO_PATH se resuelve correctamente o los fallbacks funcionan.
#  La función render_portada es la más relevante para el logo en el cuerpo principal)
...
def render_portada(data):
    ...
    # Usar la variable global pre-calculada logo_exists_at_path
    if logo_exists_at_path:
        try:
            # st.image maneja objetos Image de Pillow o rutas de archivo directamente
            st.image(LOGO_PATH, width=250) # Intenta usar la ruta directamente
        except Exception as e: # Captura errores más genéricos si st.image falla con la ruta
            st.markdown(f"<p style='font-size: 0.8em; color: {COLOR_TEXTO_SUTIL};'>(Error al mostrar logo de portada con st.image: {e}. Ruta: {LOGO_PATH})</p>", unsafe_allow_html=True)
            print(f"ERROR render_portada (st.image): {e} con ruta {LOGO_PATH}")
    else:
        # Este mensaje se mostrará si logo_exists_at_path es False
        st.markdown(f"<p style='font-size: 0.8em; color: {COLOR_TEXTO_SUTIL};'>(Logo de portada no encontrado en ruta: '{LOGO_PATH}')</p>", unsafe_allow_html=True)
        print(f"INFO render_portada: logo_exists_at_path es False. Ruta: {LOGO_PATH}")
...
# ... (resto de las funciones render_glosario, render_resumen_ejecutivo, etc., SIN CAMBIOS respecto a la versión anterior) ...
# COPIAR Y PEGAR LAS FUNCIONES DE RENDERIZADO DESDE render_glosario HASTA render_conclusiones DE LA RESPUESTA ANTERIOR AQUÍ
...
if st.session_state.json_data is None:
    ...
    # Mostrar logo en pantalla de bienvenida si está disponible
    if logo_base64: # Usar el base64 aquí, ya que es para HTML
        ...
        # Fallback si el logo_base64 no se pudo cargar
        st.markdown(f"<p style='text-align: center; font-size: 1.0em; color: {COLOR_TEXTO_SUTIL};'>(Logo ECO Consultores no disponible)</p>", unsafe_allow_html=True)
    ...
    if seccion_a_mostrar in mapa_renderizado:
        ...
        if data_for_section or data_key == "portada": # Portada puede no tener muchos datos pero debe renderizarse
            render_function(data_for_section)
        else:
            st.warning(f"Datos para la sección '{seccion_a_mostrar}' (clave: '{data_key}') no encontrados o vacíos en el archivo JSON.")
            render_function({}) 
    else:
